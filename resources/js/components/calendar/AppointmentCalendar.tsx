...
import { Avatar, Box, Button, ButtonGroup, Card, CardContent, Fade, IconButton, Stack, Tooltip, Typography } from '@mui/material';
            ...
            <Stack direction="row" spacing={2} alignItems="center">
                ...
                <Box>
                    ...
                    <Typography variant="body2" color="text.secondary">
                        {selectedProvider ? `Dr. ${selectedProvider.name} - Appointment Schedule` : 'Healthcare Appointment Schedule'}
                    </Typography>
                </Box>
            </Stack>
...
const EventComponent = ({ event }: { event: CalendarEvent }) => {
    ...
    return (
        ...
        >
            <Avatar sx={{ width: 16, height: 16, fontSize: '0.6rem', bgcolor: 'rgba(255,255,255,0.2)' }}>{event.title.charAt(0)}</Avatar>
...
export default function AppointmentCalendar({
    ...
    const events: CalendarEvent[] = appointments.map((appointment) => {
        ...
        const displayName = userRole === 'client' ? `Dr. ${appointment.provider.name}` : appointment.user.name;
    ...
    const slotPropGetter = (date: Date) => {
        ...
        for (const appointment of appointments) {
            const appointmentDate = appointment.date.includes('T') ? appointment.date.split('T')[0] : appointment.date;
        ...
        if (matchingAppointment) {
            const displayName = userRole === 'client' ? `Dr. ${matchingAppointment.provider.name}` : `${matchingAppointment.user.name}`;

            const statusColor =
                matchingAppointment.status === 'confirmed' ? '#2ECC71' : matchingAppointment.status === 'cancelled' ? '#E74C3C' : '#F39C12';
                        ...
                        <Calendar
                            ...
                            components={{
                                toolbar: (props: ToolbarProps<CalendarEvent, object>) => (
                                    <CustomToolbar {...props} selectedProvider={selectedProvider} />
                                ),
                                event: EventComponent,
                            }}
                            formats={{
                                timeGutterFormat: 'h:mm A',
                                eventTimeRangeFormat: ({ start, end }) => `${moment(start).format('h:mm A')} - ${moment(end).format('h:mm A')}`,
