...
const ECG: React.FC<ECGProps> = ({ width = '100%', height = 200, speed = 1, color = '#00FF00', className = '', bpm = 72 }) => {
    ...
    const generateECGPath = (startX: number = 0): string => {
        ...
        for (let cycle = 0; cycle < totalCycles; cycle++) {
            ...
            const prSegmentPoints = [[cycleStartX + 50, baselineY + randomAmplitude()]];
            ...
            const stSegmentPoints = [[cycleStartX + 85, baselineY - 2 + randomAmplitude()]];
            ...
            const allPoints = [...pWavePoints, ...prSegmentPoints, ...qrsPoints, ...stSegmentPoints, ...tWavePoints, ...baselinePoints];
    ...
    return (
        <div className={`relative overflow-hidden rounded-lg border border-gray-800 bg-black ${className}`} style={{ width, height }}>
            <svg className="absolute inset-0 h-full w-full opacity-30" xmlns="http://www.w3.org/2000/svg">
                <defs>
                    <pattern id="majorGrid" width="25" height="25" patternUnits="userSpaceOnUse">
                        <path d="M 25 0 L 0 0 0 25" fill="none" stroke={color} strokeWidth="0.8" opacity="0.4" />
                    </pattern>

                    <pattern id="minorGrid" width="5" height="5" patternUnits="userSpaceOnUse">
                        <path d="M 5 0 L 0 0 0 5" fill="none" stroke={color} strokeWidth="0.3" opacity="0.2" />
                    </pattern>
                </defs>
            ...
            <svg className="absolute inset-0 h-full w-full" viewBox="0 0 800 200" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
                <defs>
                ...
                <g mask="url(#fadeMask)">
                    <path d={ecgPath1} fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.3" />
                    <path d={ecgPath2} fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.3" />
                    <path d={ecgPath3} fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.3" />
                </g>

                <line x1={sweepPosition} y1="0" x2={sweepPosition} y2="200" stroke={color} strokeWidth="2" opacity="0.8" filter="url(#glow)" />

                <g>
                    <defs>
                        <clipPath id={`sweepClip`}>
                            <rect x={sweepPosition - 50} y="0" width="50" height="200" />
                        </clipPath>
                    </defs>
            ...
            <div className="pointer-events-none absolute inset-0">
                <div className="absolute top-0 right-0 left-0 h-1 bg-gradient-to-b from-gray-700 to-transparent opacity-50"></div>

                <div className="absolute right-0 bottom-0 left-0 h-1 bg-gradient-to-t from-gray-700 to-transparent opacity-50"></div>

                <div className="absolute top-0 bottom-0 left-0 w-1 bg-gradient-to-r from-gray-700 to-transparent opacity-50"></div>
                <div className="absolute top-0 right-0 bottom-0 w-1 bg-gradient-to-l from-gray-700 to-transparent opacity-50"></div>
            ...
            <div className="absolute top-2 right-2 font-mono text-xs text-green-400 opacity-75 transition-all duration-500">{bpm} BPM</div>

            <div className="absolute top-2 left-2 font-mono text-xs text-green-400 opacity-75">LEAD II</div>
        </div>
    );
};
