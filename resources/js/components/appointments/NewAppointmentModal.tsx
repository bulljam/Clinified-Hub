...
export default function NewAppointmentModal({ open, onClose, providers, allAppointments, currentUser }: NewAppointmentModalProps) {
    ...
    const getProviderAppointmentsForDate = (providerId: string, date: string): Appointment[] => {
        ...
        return allAppointments.filter(
            (appointment) =>
                appointment.provider_id === parseInt(providerId) && appointment.date.substring(0, 10) === date && appointment.status !== 'cancelled',
        );
    };
    ...
    const getPatientAppointmentsForDate = (date: string): Appointment[] => {
        ...
        return allAppointments.filter(
            (appointment) =>
                appointment.user_id === currentUser.id && appointment.date.substring(0, 10) === date && appointment.status !== 'cancelled',
        );
    };
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                ...
                                {data.provider_id && data.date && hasAppointmentWithProvider(data.provider_id, data.date) && (
                                    <Alert severity="warning" sx={{ mb: 2 }}>
                                        You already have an appointment with this provider on the selected date. Please choose a different provider or
                                        date.
                                    </Alert>
                                )}
                                    ...
                                    <FormControl fullWidth error={!!errors.time}>
                                        ...
                                        {(errors.time ||
                                            ...
                                            <Box sx={{ mt: 1, fontSize: '0.75rem', color: errors.time ? 'error.main' : 'text.secondary' }}>
                                                {errors.time ||
                                                    (!data.provider_id || !data.date
                                                        ? 'Please select provider and date first'
                                                        : hasAppointmentWithProvider(data.provider_id, data.date)
                                                          ? 'Cannot book multiple appointments with same provider on same date'
                                                          : getAvailableTimeSlots().length === 0
                                                            ? 'No available time slots for selected date'
