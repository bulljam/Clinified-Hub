...
it('prevents patient from booking multiple appointments with same provider on same date', function () {
    ...
    // Try to create second appointment with same provider on same date
    ...
    // Check if appointment was actually created (it shouldn't be)
    ...
    // Verify second appointment was not created
...
it('allows patient to book with same provider on different date', function () {
    ...
    // Create second appointment with same provider on different date
    $response = $this->actingAs($patient)->post('/appointments', [
        'provider_id' => $provider->id,
        'date' => '2025-09-02',
        'time' => '14:00',
    ...
    // Verify second appointment was created
...
it('allows patient to book with different provider on same date', function () {
    ...
    // Create first appointment with provider 1
    Appointment::factory()->create([
        'user_id' => $patient->id,
    ...
    // Create second appointment with different provider on same date
    $response = $this->actingAs($patient)->post('/appointments', [
...
it('allows patient to book with same provider if previous appointment is cancelled', function () {
    ...
    // Try to create new appointment with same provider on same date
    $response = $this->actingAs($patient)->post('/appointments', [
        'provider_id' => $provider->id,
        'date' => '2025-09-01',
        'time' => '14:00',
...
it('still prevents booking same time slot for provider', function () {
    ...
    // Try to create second appointment at same time slot
